...
<%@page import="in.co.sunrays.proj3.controller.UserListCtlk"%>
...
<jsp:useBean id="dto2" class="in.co.sunrays.proj3.dto.RoleDTO" scope="request"></jsp:useBean>
 ...
 <%List l=ServletUtility.getList(request); %>
   ...
   <div class="col-sm-2">
    ...
    <div class="col-sm-2"> 
    <button type="submit" value="<%=UserListCtlk.OP_SAVE%>" class="btn btn-primary" style="margin-top: 25px;"><i class="fa fa-search" aria-hidden="true"></i>&nbsp;Search</button>
 
    <button type="submit" value="<%=UserListCtlk.OP_RESET%>" class="btn btn-primary" style="margin-top: 25px;"><i class="fa fa-refresh" aria-hidden="true"></i>&nbsp;Reset</button>
 </div>
 <div class="col-sm-3"></div>
   </div>
   <br>
...
<div class="container">
  ...
  <ul class="pager">
     ...
     <%if(pageNo==1){ %>
     <button type="submit" name="operation" value="<%=UserListCtlk.OP_PREVIOUS%>" class="previous btn btn-primary" disabled="disabled"><i  class='fas fa-caret-square-left' style="color:" aria-hidden="true"></i>&nbsp;Previous</button>
     <%}else{ %>
     
  <button type="submit" name="operation" value="<%=UserListCtlk.OP_PREVIOUS%>" class="previous btn btn-primary"><i  class='fas fa-caret-square-left' style="color:" aria-hidden="true"></i>&nbsp;Previous</button>
     ...
     <%} %>
      ...
      <button type="submit" name="operation" value="<%=UserListCtlk.OP_DELETE%>" class="btn btn-danger" disabled="disabled"><i class="fa fa-trash" aria-hidden="true"></i>Delete</button>
<%}else{ %>
<button type="submit" name="operation" value="<%=UserListCtlk.OP_DELETE%>" class="btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i>Delete</button>

<%} %>

<%if(userBean1.getRoleId()==2||userBean1.getRoleId()==4||userBean1.getRoleId()==3){ %> 
      
       <button type="submit" name="operation" value="<%=UserListCtlk.OP_NEW%>" class="btn btn-success" disabled="disabled"><i class="fa fa-plus" aria-hidden="true"></i>Add</button>
       <%}else{ %>
        <button type="submit" name="operation" value="<%=UserListCtlk.OP_NEW%>" class="btn btn-success"><i class="fa fa-plus" aria-hidden="true"></i>Add</button>
     <%} %>
       
      <%if(i>0){ %>  
       <button type="submit" name="operation" value="<%=UserListCtlk.OP_NEXT%>" class="next btn btn-primary" >Next &nbsp;<i  class='fas fa-caret-square-right' style="color:" aria-hidden="true"></i></button></center>
       <%}else{ %>
       
          <button type="submit" name="operation" value="<%=UserListCtlk.OP_NEXT%>" class="next btn btn-primary" disabled="disabled">Next &nbsp;<i  class='fas fa-caret-square-right' style="color:" aria-hidden="true"></i></button></center>
   <%} %>
       </center>
  </ul>
</div>
